...
| `multi-agent-quickstart` | Build Multi-Agent AI Systems with Open Protocols | One lab containing local, hands-on OpenShift, and advanced blueprint tracks. Track 2 runs a real workflow and has the learner apply, verify, and roll back an agent policy. The Arena topology and base runtime are certified for 25 seats; the v0.2.5 hands-on delta is GREEN-live at one seat and still needs an exact-image 25-seat regression before event use. Optional advanced integrations, durable image supply, and public access remain gated |
